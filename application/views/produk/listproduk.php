    ...
    <div class="row bg-primary2 my-2 rounded-1 p-3">
      <div class="col-lg py-2 search-top">
        <form class="input-group" action="" method="get">
            <input class="border border-1 px-3" style="border-radius: 7px 0 0 7px"  type="search" name="keyword" style="width: 360px;" placeholder="Keyword.." value="<?= html_escape($keyword) ?>" maxlength="32" />
          <?php
          if ($keyword == null) {
            $keyword == 'all';
          }
          ?>
          <button class="btn btn-primary" value="<?= $keyword ?>" id='keyword' type="submit">Search</button>
        </form>
      </div>
    ...
    <div class="row">
      <div class="col-lg-2">
        <div class="row nav nav-pills mb-5 bg-primary2" id="v-pills-tab" role="tablist" aria-orientation="vertical">
          <button class="col-12 nav-link rounded-0 shadow-sm active" id="v-pills-all-tab" data-bs-toggle="pill"
            data-bs-target="#v-pills-all" type="button" role="tab" aria-controls="v-pills-all" aria-selected="true">
            Semua
          </button>
          <?php foreach ($produk as $produk) {
                        if ($produk->jenis_berita == 'Produk') {
                            $kategori = $this->db->get_where('kategori', array('id_kategori' => $produk->id_kategori))->row() ?>
          <button class="col-12 nav-link rounded-0 shadow-sm"
            id="v-pills-<?= strtolower($kategori->nama_kategori) ?>-tab" data-bs-toggle="pill"
            data-bs-target="#v-pills-<?= strtolower($kategori->nama_kategori) ?>" type="button" role="tab"
            aria-controls="v-pills-<?= strtolower($kategori->nama_kategori) ?>" aria-selected="false">
        ...
        <div class="tab-content" id="v-pills-tabContent">
          ...
          <div class="tab-pane fade show active" id="v-pills-all" role="tabpanel" aria-labelledby="v-pills-all-tab"
            ...
            <div class="row pembungkus">
              <?php
              foreach ($produksearch as $produk) {
                if ($produk->jenis_berita == 'Produk') { ?>
              <div class="col-lg-3 py-2">
                <div class="card border-0 shadow-sm">
                  ...
                  <div class="card-body">
                    <a href="<?= base_url('Produk/detail/' . $produk->slug_berita) ?>"
                      class="text-decoration-none stretched-link small fw-light">
          ...
          <div class="tab-pane fade" id="v-pills-<?= strtolower($kategori->nama_kategori) ?>" role="tabpanel"
            aria-labelledby="v-pills-<?= strtolower($kategori->nama_kategori) ?>-tab" tabindex="0">
            <div class="row pembungkus">
              <?php foreach ($produkByCategory as $produkByCategory) { ?>
              <div class="col-lg-3 py-2">
                <div class="card border-0 shadow-sm">
                  <img src="<?php echo base_url('assets/upload/image/thumbs/' . $produkByCategory->gambar); ?>"
                    class="card-img-top p-4" alt="..." />
                  <div class="card-body">
