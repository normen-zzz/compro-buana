...
class Galeri extends CI_Controller
{
	...
	public function index()
	{
		...
		$data = array(
			...
			'tahun'		=> $this->galeri_model->tahun_galeri(),
			'galeri'	=> $galeri,
			'kategori'	=> $kategori,
			'isi'		=> 'galeri/list'
		);
	...
	public function detail($id_kategori_galeri)
	{
		...
		// Galeri dan paginasi
		$this->load->library('pagination');
		$config['base_url'] 		= base_url() . 'galeri/detail/';
		$config['total_rows'] 		= count($this->galeri_model->total_galeri());
		$config['use_page_numbers'] = TRUE;
		$config['num_links'] 		= 5;
		$config['uri_segment'] 		= 3;
		$config['per_page'] 		= 12;
		$config['first_url'] 		= base_url() . 'galeri/detail/';
		$this->pagination->initialize($config);
		$page 		= ($this->uri->segment(3)) ? ($this->uri->segment(3) - 1) * $config['per_page'] : 0;
		$galeri 	= $this->galeri_model->showGaleriByCategory($id_kategori_galeri);
		// End paginasi
